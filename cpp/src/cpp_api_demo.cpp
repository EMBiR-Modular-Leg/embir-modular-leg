...
void Run(std::ofstream& data_file, DemoSettings& demoset, Actuator& a1,         Actuator& a2) {
  ...
  // ** TEST PERIOD **
  const auto period =
      std::chrono::microseconds(static_cast<int64_t>(demoset.period_s * 1e6));
  auto next_cycle = std::chrono::steady_clock::now() + period;

  // ** TERMINAL STATUS UPDATE PERIOD **
  const auto status_period = std::chrono::microseconds(demoset.status_period_us);
  const auto grp_sampling_period = std::chrono::microseconds(demoset.grp_sampling_period_us);
